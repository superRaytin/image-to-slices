...
function ImageToSlices(lineXArray, lineYArray, options) {
  ...
  if (!this.options.saveToDir) {
    throw new Error('Save directory must be specified');
  }
...
ImageToSlices.prototype.clipChild = function(children, parentBlockIndex, callback) {
  ...
  function cropper() {
    ...
    var saveImageName = 'section-' + parentBlockIndex + '-' + childBlockIndex + '.' + imageFormat;
...
ImageToSlices.prototype.clip = function(blocks, callback) {
  ...
  if (blocks.length) {
    ...
    function cropper() {
      ...
      var saveImageName = 'section-' + blockIndex + '.' + imageFormat;
      var saveFileName = Path.join(exportPath, saveImageName);
      ...
      if (item.children) {
        ...
        clipper
            ...
            // 将挖空的大背景区域保存为文件
            .toFile(saveFileName, function() {
              self.clipChild(item.children, blockIndex - 1, cropper);
            });
      } else {
        // 裁切图片后保存为文件
        clipper
